...
public class EncuestaDtoFacadeREST {
    ...
    @Produces({MediaType.APPLICATION_JSON})
    public List<PreguntaDto> findPreguntas(@PathParam("idencuesta") Long id) {
        Encuesta encuesta = this.encuestaFacade.find(id);
        List<Pregunta> lista = encuesta.getPreguntaList();
        List<PreguntaDto> listaDto = new ArrayList();        
        if (lista != null && !lista.isEmpty()) {
            for (Pregunta preg:lista) {
                listaDto.add(preg.crearDTO());
            }            
        }
        return listaDto;            
    }        
